...
enum { PLAYERFRONT, ENEMY_1, ENEMY_2, ENEMY_3, BLLTSPRT, LIVESPRITE};
...
void drawGame() {
    ...
    for (int i = 0; i < ENEMY1COUNT; i++) {
        if(enemy1[i].active) {
            ...
            shadowOAM[i+1].attr2 = ATTR2_PALROW(0) | ATTR2_TILEID(enemy1[i].curFrame * 2, enemy1[i].aniState *2);
        }
    }
    ...
    for (int i = 0; i < BULLETCOUNT; i++) {
        if(bullet[i].active) {
            ...
            shadowOAM[i+21].attr1 = bullet[i].col | ATTR1_SMALL;
            shadowOAM[i+21].attr2 = ATTR2_PALROW(0) | ATTR2_TILEID(bullet[i].curFrame * 2, bullet[i].aniState *2);
        }
    }
    waitForVBlank();
