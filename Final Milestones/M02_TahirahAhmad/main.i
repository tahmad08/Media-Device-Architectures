...
# 1 "spritesht.h" 1
# 21 "spritesht.h"
extern const unsigned short spriteshtTiles[16384];


extern const unsigned short spriteshtPal[256];
# 13 "main.c" 2
...
void initialize() {
    ...
    DMANow(3, spriteshtPal, ((unsigned short *)0x5000200), 512/2);
    DMANow(3, spriteshtTiles, &((charblock *)0x6000000)[4], 32768/2);
...
void start() {
    ...
    DMANow(3, startscrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, startscrnTiles, &((charblock *)0x6000000)[0], 2528/2);
    DMANow(3, startscrnMap, &((screenblock *)0x6000000)[28], 2048/2);
    (*(volatile unsigned short*)0x4000008) = ((0)<<2) | ((28)<<8) | (0<<7) | (0<<14);
...
void goToGame() {
    ...
    DMANow(3, gamescrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, gamescrnTiles, &((charblock *)0x6000000)[0], 224/2);
    DMANow(3, gamescrnMap, &((screenblock *)0x6000000)[28], 2048/2);
...
void instr() {
    ...
    DMANow(3, instrscrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, instrscrnTiles, &((charblock *)0x6000000)[0], 3520/2);
    DMANow(3, instrscrnMap, &((screenblock *)0x6000000)[28], 2048/2);
    (*(volatile unsigned short*)0x4000008) = ((0)<<2) | ((28)<<8) | (0<<7) | (0<<14);
    (*(unsigned short *)0x4000000) = 0 | (1<<8);
...
void pause() {
    ...
    hideSprites();
    DMANow(3, pausescrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, pausescrnTiles, &((charblock *)0x6000000)[0], 3456/2);
    DMANow(3, pausescrnMap, &((screenblock *)0x6000000)[28], 2048/2);
...
void win() {
    ...
    DMANow(3, winscrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, winscrnTiles, &((charblock *)0x6000000)[0], 832/2);
    DMANow(3, winscrnMap, &((screenblock *)0x6000000)[28], 2048/2);
    (*(volatile unsigned short*)0x4000008) = ((0)<<2) | ((28)<<8) | (0<<7) | (0<<14);
    (*(unsigned short *)0x4000000) = 0 | (1<<8);
...
void lose() {
    ...
    DMANow(3, losescrnPal, ((unsigned short *)0x5000000), 512/2);
    DMANow(3, losescrnTiles, &((charblock *)0x6000000)[0], 1280/2);
    DMANow(3, losescrnMap, &((screenblock *)0x6000000)[28], 2048/2);
    (*(volatile unsigned short*)0x4000008) = ((0)<<2) | ((28)<<8) | (0<<7) | (0<<14);
    (*(unsigned short *)0x4000000) = 0 | (1<<8);
...
void updatePlayer() {
    if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))
        ...
        && player.col + player.width < 240 - player.cdel) {
    ...
    if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && player.bulletTimer >= 30) {
        fireBullet();
...
void updateBullet(BULLET* b) {
    ...
    if (b->active) {
        if (b->row - 5 > 0) {
            ...
            b->active = 0;
            b->curFrame = 0;
            shadowOAM[b->shadowOam].attr0 |= (2<<8);
        }
    }
}
...
void updateEnemy1(ENEMY1 *e) {
    ...
    if (e->active) {
        if(e->row < 160 && e->col < 240) {
            e->row += e->rdel;
            if (e->row + e->height >= 160 - 2) {
                e->active = 0;
                shadowOAM[e->shadowOam].attr0 |= (2<<8);
                enemy1onscrn--;
            }
    ...
    for (int i = 0; i < 10; i++) {
        if (collision (bullet[i].row, bullet[i].col, bullet[i].height, bullet[i].width, e->row, e->col, e->height, e->width)) {
            ...
            e->aniState = ENEMY_1;
            shadowOAM[e->shadowOam].attr0 |= (2<<8);
            shadowOAM[bullet[i].shadowOam].attr0 |= (2<<8);
        }
    }
...
void drawGame() {
    ...
    for (int i = 0; i < 40; i++) {
        if(enemy1[i].active) {
            shadowOAM[i+1].attr0 = enemy1[i].row | (0<<13) | (0<<14);
            shadowOAM[i+1].attr1 = enemy1[i].col | (0<<14);
            shadowOAM[i+1].attr2 = ((0)<<12) | ((enemy1[i].curFrame)*32+(enemy1[i].aniState));
        }
    }

    for (int i = 0; i < 10; i++) {
        if(bullet[i].active) {
            shadowOAM[i+21].attr0 = bullet[i].row | (0<<13) | (0<<14);
            shadowOAM[i+21].attr1 = bullet[i].col | (0<<14);
            shadowOAM[i+21].attr2 = ((0)<<12) | ((bullet[i].curFrame)*32+(bullet[i].aniState));
        }
    }
    waitForVBlank();
