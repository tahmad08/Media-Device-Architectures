...
void initialize() {
    ...
    DMANow(3, startscrnTiles, &((charblock *)0x6000000)[0], 2496/2);
...
void game() {
    ...
    DMANow(3, gamescrnTiles, &((charblock *)0x6000000)[0], 672/2);
...
void pause() {
    ...
    DMANow(3, pausescrnTiles, &((charblock *)0x6000000)[0], 4224/2);
...
void updatePlayer() {
    ...
    if ((!(~(oldButtons)&((1<<0))) && (~buttons & ((1<<0)))) && !bullet.active) {
...
void drawGame() {
    ...
    shadowOAM[1].attr0 = bullet.row | (0<<13) | (0<<14);
    shadowOAM[1].attr1 = bullet.col | (0<<14);
    shadowOAM[1].attr2 = ((0)<<12) | ((bullet.curFrame)*32+(bullet.aniState));


    for (int i = 0; i < 35; i++) {
        if(enemy[i].active) {
            shadowOAM[i+2].attr0 = enemy[i].row | (0<<13) | (0<<14);
            shadowOAM[i+2].attr1 = enemy[i].col | (0<<14);
            shadowOAM[i+2].attr2 = ((0)<<12) | ((enemy[i].curFrame)*32+(enemy[i].aniState));
        }
    }
    for (int i = 0; i < 3; i++) {
        if(lives[i].active) {
            shadowOAM[i+38].attr0 = lives[i].row | (0<<13) | (0<<14);
            shadowOAM[i+38].attr1 = lives[i].col | (0<<14);
            shadowOAM[i+38].attr2 = ((0)<<12) | ((lives[i].curFrame)*32+(lives[i].aniState));
        }
    }
