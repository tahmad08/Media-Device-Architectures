...
void drawBall() {
    if (ball.active) {
        drawRect(ball.oldRow, ball.oldCol, ball.height, ball.width, 0);
...
void updateBall() {
    ...
    if (ball.row <= 0)
        ball.rdel *= -1;
    if (ball.col <= 0 || ball.col + ball.width - 1 >= 240 -1)
        ball.cdel *= -1;
...
void drawBrick(BRICK* b) {
    ...
    if(b->active) {
        ...
        drawRect(b->row, b->col, b->height, b->width, 0);
        b->erased = 1;
    }
