...
enum {BLACKID=(256-8), BLUEID, GREENID, REDID, WHITEID, GRAYID, YELLOWID, LIGHTGREENID};
...
# 1 "badBoi.h" 1
# 21 "badBoi.h"
extern const unsigned short badBoiBitmap[72];


extern const unsigned short badBoiPal[256];
# 5 "game.c" 2
# 1 "goodBoi.h" 1
# 21 "goodBoi.h"
extern const unsigned short goodBoiBitmap[72];


extern const unsigned short goodBoiPal[256];
# 6 "game.c" 2
# 1 "blueBoi.h" 1
# 21 "blueBoi.h"
extern const unsigned short blueBoiBitmap[72];


extern const unsigned short blueBoiPal[256];
# 7 "game.c" 2
# 1 "catcher.h" 1
# 21 "catcher.h"
extern const unsigned short catcherBitmap[192];


extern const unsigned short catcherPal[256];
# 8 "game.c" 2
...
void initGame() {
 ...
 j = 0;
    ...
    unsigned short colors[8] = {0, ((0) | (0)<<5 | (31)<<10), ((0) | (31)<<5 | (0)<<10), ((31) | (0)<<5 | (0)<<10), ((31) | (31)<<5 | (31)<<10), ((16) | (16)<<5 | (16)<<10), ((31) | (31)<<5 | (0)<<10), ((16) | (31)<<5 | (16)<<10)};

    DMANow(3, catcherPal, ((unsigned short *)0x5000000), 256);
...
void updateBullet(BULLET* b) {
 ...
 if (b->active) {
  if (b->row + b->height-1 >= 0 && b->col + b->cdel > 0 && b->col + b->cdel < 240 -1) {
   b->row += b->rdel;
            b->col += b->cdel;
...
void initBalls() {
 ...
 for (int i = 0; i < 45; i++) {
  ...
  balls[i].width = 12;
  balls[i].row = 0;
  balls[i].col = (rand() + rando % (240 + 100)) + 100;
  balls[i].rdel = 2;
...
void reInitBall() {
 for (int i = 0; i < 45; i++) {
  if (!(balls[i].active)) {
   ...
   balls[i].active = 1;
   balls[i+1].row = (rand() % 20) + 5;
   balls[i+1].col = rand() % 130 + 10;
   balls[i+1].active = 1;
   j++;
   break;
